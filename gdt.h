...
#ifndef _TINIX_GDT_H_
#define _TINIX_GDT_H_
...
typedef struct {
  uint16 seglimitlow;       // limit (bits 0-15)
  uint16 baseaddrlow;       // base (bits 0-15)
  uint8 baseaddrmid;        // base (bits 16-25)
  uint8 accessmode;         // access
  uint8 flaglimithigh;      // limit (bits 16-19) | flags
  uint8 baseaddrhigh;       // base (bits 24-31)
} __attribute__((packed)) gdtentry_t;

typedef struct {
  uint16 limit;               // sizeof(gdt_t) - 1
...
void load_gdt(gdtdesc_t *, uint16, uint16);
void gdt_init(void);
